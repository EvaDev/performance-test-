...
async def run_performance_test(
    ...
    def sign_transaction(transaction, account):
        """Sign a transaction synchronously."""
        signature = account.signer.sign_transaction(transaction)
        # Use dataclasses.replace to update signature (same as starknet_py does internally)
        return dataclasses.replace(transaction, signature=signature)
    
    # Sign in parallel using thread pool (for CPU-bound operations)
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(50, len(valid_prep))) as executor:
        sign_futures = [
            executor.submit(sign_transaction, tx, acc)
    ...
    actual_ops = total_ops_executed / total_duration if total_duration > 0 else 0
    ...
    logger.info("=" * 60)
    logger.info(f"Actual OPS: {actual_ops:.2f}")
    logger.info(f"Operations Executed: {total_ops_executed}")
    logger.info(f"Total Duration: {total_duration:.2f}s")
    logger.info(f"Successful: {len(successful)}")
    logger.info(f"Failed: {len(failed)}")
    logger.info(f"Avg TX Duration: {avg_tx_duration:.2f}s")
    logger.info(f"Parallel Operations: {parallel_ops}")
    logger.info("=" * 60)
