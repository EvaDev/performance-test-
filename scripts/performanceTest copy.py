...
load_dotenv()
NODE_URL = os.getenv("NODE_URL", "https://starknet-sepolia.infura.io/v3/eaa346e675484d7889e3b849fdba7976")
...
#NODE_URL = os.getenv("NODE_URL", "https://starknet-sepolia.public.blastapi.io/rpc/v0_9")
...
async def get_contract(address, client):
    ...
    # For known proxies like STRK, resolve implementation
    if address == STRK_TOKEN_ADDRESS:
        proxy_contract = Contract(address=address, abi=abi, provider=client)
        implementation_hash = await proxy_contract.functions["implementation"].call()
        impl_class = await client.get_class_by_hash(implementation_hash[0], block_number="latest")
        abi = impl_class.abi
...
async def fetch_balances(test_accounts):
    coros = [get_strk_balance(client, acc["address"]) for acc in test_accounts]
