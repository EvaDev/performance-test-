...
async def run_performance_test(
    ...
    # Get starting nonces for all accounts in parallel
    # Use "latest" block tag for v0.8.1 RPC compatibility (doesn't support "pending")
    nonce_tasks = [acc.get_nonce(block_number="latest") for acc in set(op["account"] for op in operations)]
    starting_nonces = await asyncio.gather(*nonce_tasks)
    account_to_starting_nonce = dict(zip([acc.address for acc in set(op["account"] for op in operations)], starting_nonces))
    
    # Prepare and sign transactions
    # Manually estimate fees first (using "latest" block tag for v0.8.1 compatibility)
    # Then use those bounds for all transactions
    logger.info("Preparing and signing transactions...")
    prep_start_time = time.time()
    
    update_balance_selector = get_selector_from_name("update_balance")
    
    # Estimate fees manually using "latest" block tag (v0.8.1 doesn't support "pending")
    ...
    for addr, ops in account_ops.items():
        starting_nonce = account_to_starting_nonce[addr]
    ...
    # All transactions are already prepared (synchronously)
    # Filter valid transactions
    valid_prep = []
    for tx, account, op_id in prep_tasks_list:
        if tx:  # All manually constructed transactions should be valid
